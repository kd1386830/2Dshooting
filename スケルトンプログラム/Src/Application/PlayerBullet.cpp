...
void C_PlayerBullet::shot(C_Player& player)
{
	for (int i = 0;i < BulletNum;i++)
	{
		if (!m_PBulletFlg[i])
		{
			m_PBulletPos[i] = player.GetPlayerPos();

			m_PBulletMoveX[i] = cos(player.GetAngle()) * m_PBulletSpeed;
			m_PBulletMoveY[i] = sin(player.GetAngle()) * m_PBulletSpeed;
